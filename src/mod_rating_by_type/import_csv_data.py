...
from stats.models import Object, Score
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        modded_score = base_dir.joinpath('mod_rating_by_type').joinpath('score.csv')
        score_dict = {}
        update_score(score, score_dict)
        update_score(modded_score, score_dict)
...
def update_score(score_csv, score_dict):
    with score_csv.open(encoding='utf-8') as file_csv:
        for row in csv.DictReader(file_csv):
